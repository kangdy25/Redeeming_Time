...
async function request<T>(path: string, init: ApiRequestInit = {}): Promise<T> {
  ...
  const authorization = skipAuthorization ? {} : useAuthStore().authorizationHeader;
...
export const apiClient = {
  ...
  createCalendar: async (payload: CalendarPayload) => {
    ...
    planner.syncPlanner({ calendars: [...planner.calendars, calendar] });
    return calendar;
  },
  ...
  createCategory: async (payload: CategoryPayload) => {
    ...
    const planner = usePlannerStore();
    planner.syncPlanner({ categories: [...planner.categories, category] });
    return category;
  },
  updateCategory: async (categoryId: number, payload: Partial<CategoryPayload>) => {
    ...
    const planner = usePlannerStore();
    planner.syncPlanner({
      categories: planner.categories.map((item) => (item.id === category.id ? category : item)),
    });
    return category;
  },
  deleteCategory: async (categoryId: number) => {
    ...
    const planner = usePlannerStore();
    planner.syncPlanner({
      categories: planner.categories.filter((category) => category.id !== categoryId),
  ...
  createEvent: async (payload: EventPayload) => {
    ...
    const planner = usePlannerStore();
    planner.syncPlanner({ events: [...planner.events, event] });
    return event;
  },
  updateEvent: async (eventId: number, payload: Partial<EventPayload>) => {
    ...
    const planner = usePlannerStore();
    planner.syncPlanner({
      events: planner.events.map((item) => (item.id === event.id ? event : item)),
    });
    return event;
  },
  deleteEvent: async (eventId: number) => {
    ...
    const planner = usePlannerStore();
    planner.syncPlanner({
      events: planner.events.filter((event) => event.id !== eventId),
    });
  },
  ...
  createTask: async (payload: TaskPayload) => {
    ...
    const planner = usePlannerStore();
    planner.syncPlanner({ tasks: [...planner.tasks, task] });
    return task;
  },
  ...
  editTask: async (taskId: number, payload: Partial<TaskPayload>) => {
    ...
    const planner = usePlannerStore();
    planner.syncPlanner({
      tasks: planner.tasks.map((item) => (item.id === task.id ? task : item)),
    });
    return task;
  },
  deleteTask: async (taskId: number) => {
    ...
    const planner = usePlannerStore();
    planner.syncPlanner({
      tasks: planner.tasks.filter((task) => task.id !== taskId),
    });
  },
  updateTaskTargetDate: async (task: Task, targetDate: string) => {
    ...
    const planner = usePlannerStore();
    planner.syncPlanner({
      tasks: planner.tasks.map((item) => (item.id === updatedTask.id ? updatedTask : item)),
    });
    return updatedTask;
  },
